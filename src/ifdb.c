...
InfluxInfo *ifdb_alloc(char *host,
                       uint32_t port,
                       char *db,
                       char *user,
                       char *pass,
...
void ic_check(long adding) {
  ...
  if (output == (char *)0) {
    // if ((data->output = (char *)realloc(data->output, data->output_size +
    // MEGABYTE))
    // if ((data->output = (char *)realloc(data->output, data->output_size +
    // MEGABYTE)) == NULL) {
...
void ic_measure(char *section, InfluxInfo *info) {
  ...
  // data->output_char += sprintf(&data->output[data->output_char], "%s,%s ",
  // section, info->influx_tags);
  output_char +=
...
void ic_sub(char *resource, InfluxInfo *info) {
  ...
  // ic_check(strlen(data->saved_section) + strlen(info->influx_tags) +
  // strlen(data->saved_sub) +
  //         strlen(resource) + 9);

  ic_check(strlen(saved_section) + strlen(info->influx_tags) +
           strlen(saved_sub) + strlen(resource) + 9);
